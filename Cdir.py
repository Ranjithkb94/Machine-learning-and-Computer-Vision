...
def get_original_image_name(folderLocation=os.getcwd()+ r"/gtdb//" + sys.argv[1] ):
    try:
        os.chdir(folderLocation)
        listOfFile = glob.glob("*Original_raw.png")
        return listOfFile
    except OSError as o:
...
def get_gt_image_name(folderLocation=os.getcwd()+ r"/gtdb//" + sys.argv[1]):
    try:
        os.chdir(folderLocation)
        listOfFile = glob.glob("*Original_gt.png")
        return listOfFile
    except OSError as o:
        print("OSError: {0}".format(o))
        return []
